...
- (UIDatePicker *)datePicker {
    if (!_datePicker) {
        ...
        // 设置时间范围
        if (_minDateStr) {
            NSDate *minDate = [format dateFromString:_minDateStr];
            _datePicker.minimumDate = minDate;
        }
        if (_maxDateStr) {
            NSDate *maxDate = [format dateFromString:_maxDateStr];
        ...
        if (_selectValue) {
            ...
            format.dateFormat = @"yyyy-MM-dd";
            // NSString * -> NSDate *
            NSDate *date = [format dateFromString:_selectValue];
            // 防崩处理,如果为空取现在时间
            if (date == nil) {
...
//        NSData *da = [self toDateWithDateString:_selectValue];
