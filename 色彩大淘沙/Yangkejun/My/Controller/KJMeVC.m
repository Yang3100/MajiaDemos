    ...
    if ([kind isEqualToString:CHTCollectionElementKindSectionFooter]) {
        ...
        if (indexPath.section==0) {
            if (reusableView.subviews.count == 0) {//加一个限制，避免无限创建新的
                ...
                UIImage *cover_image = [UIImage imageWithContentsOfFile:cover_path]!=nil ? [UIImage imageWithContentsOfFile:cover_path]:GetImage(@"myBgImage");
                ...
                [_nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.centerY.mas_equalTo(headerImageView);
    ...
    if (indexPath.section == 1) {
        switch (indexPath.row) {
            ...
            case 5:{ // 关于我们
                ...
                break;
            case 6:{ //   给我们评分
                [KJTools jumpToAppReviewPageWithAppId:kAppID];
            }
                break;
            case 7:{ // 清除缓存
