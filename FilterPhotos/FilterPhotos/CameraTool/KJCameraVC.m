...
@interface KJCameraVC ()<UIGestureRecognizerDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,ClipPhotoDelegate>
...
- (void)createdTool{
    CGFloat toolH = 100;
    self.toolView = [[UIView alloc] initWithFrame:CGRectMake(0, KHEIGHT - toolH - 64, KWIDTH, toolH)];
    self.toolView.backgroundColor = [UIColor blackColor];
    self.toolView.alpha = 0.8;
    [self.view addSubview:self.toolView];
    
    UIButton *cameraBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    cameraBtn.frame = CGRectMake((KWIDTH - 54)/2.0, (toolH - 54)/ 2.0, 54, 54);
    ...
    photoBtn.frame = CGRectMake(((KWIDTH / 2.0) - 30)/2.0, ((toolH) - 30)/ 2.0, 30, 30);
    ...
    lampBtn.frame = CGRectMake(((KWIDTH / 2.0) - 30)/2.0 + (KWIDTH /2.0), ((toolH) - 30)/ 2.0, 30, 30);
...
- (void)takePhoto:(UIImage *)image{
    if (self.delegate && [self.delegate respondsToSelector:@selector(kj_cameraTakePhoto:)]) {
        [self dismissViewControllerAnimated:YES completion:nil];
        [self.delegate kj_cameraTakePhoto:image];
    }
}
