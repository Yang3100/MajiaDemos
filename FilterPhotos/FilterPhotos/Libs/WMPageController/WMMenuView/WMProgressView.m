...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat currentWidth = currentFrame.size.width-20;
    int nextIndex = index + 1 < self.itemFrames.count ? index + 1 : index;
    CGFloat nextWidth = [self.itemFrames[nextIndex] CGRectValue].size.width-20;
    CGFloat height = self.frame.size.height;
    CGFloat constY = height / 2;
    CGFloat currentX = currentFrame.origin.x+10;
    CGFloat nextX = [self.itemFrames[nextIndex] CGRectValue].origin.x+10;
