...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        kj_w = frame.size.width;
        kj_h = frame.size.height;
        kj_w = kj_w * 3;
...
- (void)initInfo{
    ...
    _progress = 0;
    //振幅
    _wave_amplitude = kj_h / 40;
    //周期
    _wave_cycle = M_PI / (kj_w / 2);
    //两个波水平之间偏移
    _wave_h_distance = kj_w * 20;
    //两个波竖直之间偏移
    _wave_v_distance = _wave_amplitude * 0.9;
    //移动的距离，配合速率设置
...
- (void)addDisplayLinkAction{
    //创建Runloop CADisplayLink是用于同步屏幕刷新频率的计时器.
...
- (void)drawWaveColor:(UIColor *)color offsetx:(CGFloat)offsetx offsety:(CGFloat)offsety{
    ...
    CGFloat end_offY = (1 - _progress) * (kj_h) + _wave_amplitude;
    ...
    for (float next_x = 0.f; next_x <= kj_w; next_x ++) {
        //正弦函数，绘制波形  y=Asin(ωx+φ)+k
    ...
    // 波形以下内容
    [wavePath addLineToPoint:CGPointMake(kj_w, kj_h)];
    [wavePath addLineToPoint:CGPointMake(0, kj_h)];
    [color set];
    [wavePath fill];
}
