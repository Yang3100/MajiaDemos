...
- (void)saveImage{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            WMPhotoBrowserCell *currentCell = (WMPhotoBrowserCell *)[self->_collectionView cellForItemAtIndexPath:indexPath];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    if (error) {
        [MBProgressHUD showSuccess:NSLocalizedString(@"保存失败", nil)];
    } else {
        [MBProgressHUD showSuccess:NSLocalizedString(@"保存到相册成功", nil)];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell.longPressGestureBlock) {
        cell.longPressGestureBlock = ^(WMPhotoBrowserCell *cell) {
            KJAlertView *view = [[KJAlertView alloc] initWithTitle:@"保存到本地相册" Content:nil whitTitleArray:@[@"保存到相册",@"取消"] withType:@"bottom"];
