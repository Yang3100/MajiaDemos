...
//    UIImage *img = [UIImage imageWithContentsOfFile:gestureDefaultImagePath];
//    if (!img) {
//        img = [self shotView];
//        NSData *data = UIImagePNGRepresentation(img);
//        [data writeToFile:gestureDefaultImagePath atomically:YES];
//    }
//}
...
- (void)gestureEndResetMembers
{
    ...
    if (isTouchBegin) {
        isTouchBegin = NO;
        if (self.delegate && [self.delegate respondsToSelector:@selector(cicleView:type:didEndGesture:)]) {
            
            [self.delegate cicleView:self type:(CircleViewTypeSetting) didEndGesture:[self shotView]];
        }
    }
