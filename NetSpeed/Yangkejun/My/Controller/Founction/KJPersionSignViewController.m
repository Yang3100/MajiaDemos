...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = NSLocalizedString(@"Modified signature", nil) ;
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemWithImage:nil highImage:nil title:NSLocalizedString(@"Save", nil) titleColor:[UIColor whiteColor] target:self action:@selector(rightBarButtonItem:)];
    [self creatView];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if ([textView isFirstResponder]){
        if ([[[textView textInputMode] primaryLanguage] isEqualToString:@"emoji"] || ![[textView textInputMode] primaryLanguage]){
            [MBProgressHUD showError:NSLocalizedString(@"Not Emoji", nil) ];
            return NO;
        }
    }
...
- (UITextView *)remarkTextView{
    if (!_remarkTextView){
        ...
        [KJTools makeCornerRadius:Handle(5) borderColor:MainColor(.8) layer:textView.layer borderWidth:1];
