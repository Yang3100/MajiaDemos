...
+ (NSString *)formattedFileSize:(unsigned long long)size suffixLenth:(NSInteger *)length{
    ...
    size = size;
    if (size == 0)
        (void)(formattedStr = NSLocalizedString(@"0 KB",@"")), len = 2;
    else
        if (size > 0 && size < 1024)
            (void)(formattedStr = [NSString stringWithFormat:@"%qubytes", size]), (void)(*length = 2), len = 7;
        else
            if (size >= 1024 && size < pow(1024, 2))
                (void)(formattedStr = [NSString stringWithFormat:@"%.2fKB", (size / 1024.)]), len = 2;
            else
                if (size >= pow(1024, 2) && size < pow(1024, 3))
                    (void)(formattedStr = [NSString stringWithFormat:@"%.2fMB", (size / pow(1024, 2))]), len = 2;
                else
                    if (size >= pow(1024, 3))
                        (void)(formattedStr = [NSString stringWithFormat:@"%.2fGB", (size / pow(1024, 3))]), len = 2;
    if (length) {
        *length = len;
    }
    return formattedStr;
