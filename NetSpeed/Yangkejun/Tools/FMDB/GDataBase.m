...
/**
 修改数据
 */
- (BOOL)updateObjectClass:(Class)clazz keyValues:(NSDictionary *)keyValues cond:(NSString *)predicateFormat,...
{
    ...
    return [self updateTableName:tableName objectClass:clazz keyValues:keyValues cond:predicateFormat,arglist];
}

- (BOOL)updateTableName:(NSString*)tableName objectClass:(Class)clazz keyValues:(NSDictionary *)keyValues cond:(NSString *)predicateFormat,...
{
    if (keyValues.allValues.count <= 0 || !keyValues) {
        return NO;
    }
