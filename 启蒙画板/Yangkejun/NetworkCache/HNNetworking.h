 ...
 */
typedef void (^HNDownloadProgress)(int64_t bytesRead, int64_t totalBytesRead);
typedef HNDownloadProgress HNGetProgress;
typedef HNDownloadProgress HNPostProgress;
...
typedef NS_ENUM(NSInteger, HNNetworkStatus){
    ...
    kHNNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
typedef void(^HNResponseSuccess)(id response);
 ...
 */
+ (void)configRequestType:(HNRequestType)requestType
             responseType:(HNResponseType)responseType
 ...
 *	@author 黄仪标
 *
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的HNURLSessionTask对象，
 ...
 */
+ (HNURLSessionTask *)getWithUrl:(NSString *)url
                    refreshCache:(BOOL)refreshCache
                         success:(HNResponseSuccess)success
                            fail:(HNResponseFail)fail;
/*!
 *
 *  GET请求接口，若不指定baseurl，可传完整的url
 *
...
+ (HNURLSessionTask *)getWithUrl:(NSString *)url
                    refreshCache:(BOOL)refreshCache
                          params:(NSDictionary *)params
                        progress:(HNGetProgress)progress
                         success:(HNResponseSuccess)success
                            fail:(HNResponseFail)fail;
 ...
 *  @return 返回的对象中有可取消请求的API
 */
+ (HNURLSessionTask *)postWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                           params:(NSDictionary *)params
                          success:(HNResponseSuccess)success
                             fail:(HNResponseFail)fail;
...
+ (HNURLSessionTask *)postWithUrl:(NSString *)url
                     refreshCache:(BOOL)refreshCache
                           params:(NSDictionary *)params
                         progress:(HNPostProgress)progress
                          success:(HNResponseSuccess)success
                             fail:(HNResponseFail)fail;
/**
 ...
 *  @return 返回的对象中有可取消请求的API
 */
+ (HNURLSessionTask *)uploadWithImage:(UIImage *)image
                                  url:(NSString *)url
 ...
 *  @return 返回的对象中有可取消请求的API
 */
+ (HNURLSessionTask *)uploadFileWithUrl:(NSString *)url
 ...
 *  @return 返回的对象中有可取消请求的API
 */
+ (HNURLSessionTask *)downloadWithUrl:(NSString *)url
                           saveToPath:(NSString *)saveToPath
                             progress:(HNDownloadProgress)progressBlock
                              success:(HNResponseSuccess)success
                              failure:(HNResponseFail)failure;@end
