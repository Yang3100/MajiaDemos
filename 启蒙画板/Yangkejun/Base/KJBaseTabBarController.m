...
- (void)viewDidLoad {
    ...
    KJBaseNavigationController *drawNav = ({
        KJSetDrawVC *vc = [[KJSetDrawVC alloc]init];
        // 配置
        vc.title = @"画板设置";
        // 添加到导航栏的栈底控制器
        [[KJBaseNavigationController alloc] initWithRootViewController:vc];
    ...
    [self setupChildVC:homeNav title:@"简笔画" image:@"abc" selectedImage:@"abc_ed"];
    [self setupChildVC:drawNav title:@"画板设置" image:@"abc" selectedImage:@"abc_ed"];
    [self setupChildVC:meNav title:@"我的" image:@"abc" selectedImage:@"abc_ed"];
    /// 默认选择第二个
