    ...
    if ([kind isEqualToString:CHTCollectionElementKindSectionFooter]) {
        ...
        if (indexPath.section==0) {
            if (reusableView.subviews.count == 0) {//加一个限制，避免无限创建新的
                UIImageView*bgImageView = InsertImageView(reusableView, CGRectZero, GetImage(@"myBgImage"));
                ...
                UIImageView*headerImageView = InsertImageView(reusableView, CGRectZero, GetImage(@"LOGOstore_1024pt"));
                [KJTools makeCornerRadius:Handle(40) borderColor:MainColor(1) layer:headerImageView.layer borderWidth:1];
                // 添加手势
                ...
                UILabel*_nameLabel=InsertLabel(reusableView, CGRectZero, NSTextAlignmentLeft, kAppName, SystemFontSize(14), [UIColor blackColor]);
                ...
                UILabel*Label=InsertLabel(reusableView, CGRectZero, NSTextAlignmentLeft, @"这款App主要是针对于小孩子,让他们跟着慢慢学会画画,启蒙画板", SystemFontSize(12), [UIColor lightGrayColor]);
    ...
    if (indexPath.section == 1) {
        switch (indexPath.row) {
            ...
            case 6:{ //   给我们评分
                ...
                break;
            case 7:{ // 清除缓存
                KJAlertView *view = [[KJAlertView alloc] initWithTitle:@"bioat" Content:@"是否清理缓存" whitTitleArray:@[@"取消",@"确定"] withType:@"center"];
...
- (NSMutableArray*)temps{
    if (!_temps) {
        ...
        NSArray *name = @[@"修改昵称",@"修改个性签名",@"更换背景",@"设置手势密码",@"设置TouchID解锁",@"关于我们",@"给我们评分",@"清除缓存"];
        NSArray *imageName = @[@"1101026",@"1101022",@"1101045",@"1101031",@"1101030", @"1101053",@"1101018",@"1101054"];
