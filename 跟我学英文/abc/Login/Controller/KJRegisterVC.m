...
- (void)viewDidLoad {
    ...
    self.iphoneText.text = self.iphoneNum;
...
- (void)setButtonTitleCountDown{
    _timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(getCountDownText) userInfo:nil repeats:YES];
...
// 获取验证码
- (void)getDataForCode{
//    [KJNetManager getLoginCodeForIphone:self.iphoneNum completionHandler:^(id responseObj, NSError *error) {
//        if (CODE!=0) {
//            //[MBProgressHUD showError:responseObj[@"m"]];
//            [KJNetManager verificationCode:1 Phone:self.iphoneNum retry:nil];
//        }
//        if (CODE==10007) {
//            [MBProgressHUD showError:responseObj[@"m"]];
//        }
//        if (CODE==10015) {
//            //NSString *t=responseObj[@"m"];
//            [MBProgressHUD showError:@"60秒内不能重复获取"];
//        }
//        if (CODE==22) {
//            [MBProgressHUD showError:responseObj[@"m"]];
//        }
...
- (void)getDataForRegister{
//    [KJNetManager getRegisterDataWithIphoneNum:self.iphoneNum Password:self.passwordText.text Code:self.codeText.text completionHandler:^(id responseObj, NSError *error) {
//        if (CODE==0) {
//            KJUserModel *userModel = [KJUserModel modelWithJSON:responseObj[@"d"]];
//            // 保存一些参数后面使用
//            [[NSUserDefaults standardUserDefaults] setValue:userModel.access_token forKey:@"token"];
//            [[NSUserDefaults standardUserDefaults] setValue:userModel.user_id forKey:@"userId"];
//            [[NSUserDefaults standardUserDefaults] setValue:userModel.token_expires forKey:@"token_expires"];
//            [[NSUserDefaults standardUserDefaults] setValue:userModel.token_key forKey:@"token_key"];
//            [[NSUserDefaults standardUserDefaults] synchronize];
//
//            KJCompleteInfoVC *vc = [[KJCompleteInfoVC alloc] init];
//            vc.userModel = userModel;
//            vc.view.backgroundColor = [UIColor whiteColor];
//            UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:vc];
//            [self presentViewController:nav animated:YES completion:nil];
//            // 登陆成功 - 开启webSocket
//            [[NSNotificationCenter defaultCenter] postNotificationName:Login_Register_sucess object:nil userInfo:nil];
//        }
//        if (CODE==10014) {
//            [MBProgressHUD showError:responseObj[@"m"]];
//        }
...
- (UILabel*)passwordLabel{
    if (!_passwordLabel) {
        ...
        NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:@"  密码" attributes:nil];
        NSTextAttachment *textAttachment = [[NSTextAttachment alloc] initWithData:nil ofType:nil];
...
- (UIButton*)getCodeBut{
    if (!_getCodeBut) {
        ...
        _getCodeBut.layer.borderColor = UIColorFromHEXA(0x000000, 1.0).CGColor;
        [_getCodeBut setTitle:@"获取验证码" forState:UIControlStateNormal];
        [_getCodeBut setTitleColor:UIColorFromHEXA(0x000000, 1.0) forState:UIControlStateNormal];
        [_getCodeBut setTitleColor:UIColorFromHEXA(0x000000, 1.0) forState:UIControlStateSelected];
...
- (UIButton*)nextButton{
    if (!_nextButton) {
        ...
        [_nextButton addTarget:self action:@selector(thouchNext) forControlEvents:UIControlEventTouchUpInside];
