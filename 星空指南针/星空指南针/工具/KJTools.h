 ...
 *  TouchID 状态
 */
typedef NS_ENUM(NSUInteger, KJTouchIDState){
    KJTouchIDStateNotSupport = 0,          // 当前设备不支持TouchID
    KJTouchIDStateSuccess = 1,             // TouchID 验证成功
    KJTouchIDStateFail = 2,                // TouchID 验证失败
    KJTouchIDStateUserCancel = 3,          // TouchID 被用户手动取消
    KJTouchIDStateInputPassword = 4,       // 用户不使用TouchID,选择手动输入密码
    KJTouchIDStateSystemCancel = 5,        // TouchID 被系统取消 (如遇到来电,锁屏,按了Home键等)
    KJTouchIDStatePasswordNotSet = 6,      // TouchID 无法启动,因为用户没有设置密码
    KJTouchIDStateTouchIDNotSet = 7,       // TouchID 无法启动,因为用户没有设置TouchID
    KJTouchIDStateTouchIDNotAvailable = 8, // TouchID 无效
    KJTouchIDStateTouchIDLockout = 9,      // TouchID 被锁定(连续多次验证TouchID失败,系统需要用户手动输入密码)
    KJTouchIDStateAppCancel = 10,          // 当前软件被挂起并取消了授权 (如App进入了后台等)
    KJTouchIDStateInvalidContext = 11,     // 当前软件被挂起并取消了授权 (LAContext对象无效)
    KJTouchIDStateVersionNotSupport = 12   // 系统版本不支持TouchID (必须高于iOS 8.0才能使用)
};
typedef void (^StateBlock)(KJTouchIDState state,NSError *error);
...
//创建一个播放gif的ImageView
+ (CGFloat)KJ_imageViewWithGIF:(NSString*)gifStr view:(id)superView cycle:(NSInteger)cle;
...
// json字符串转字典
+ (NSDictionary *)dictionaryWitKJsonString:(NSString *)json;
