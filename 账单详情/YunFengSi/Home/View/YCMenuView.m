...
- (CGPoint)getRefPoint{
    ...
    if (absoluteRect.origin.y + absoluteRect.size.height +  menuHeight > kScreenH - 10) {
...
- (void)calculateArrowAndFrame{
    ...
    // 考虑向左右展示不全的情况，需要反向展示
    if (originX + width > kScreenW - 10) {
        originX = kScreenW - kDefaultMargin - width;
    }else if (originX < 10) {
    ...
    //如果不是根据关联视图，得算一次是否反向
    if (!_refView) {
        _isReverse = (originY + height > kScreenH - kDefaultMargin)?YES:NO;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    /// UITableViewCell 中 调整imageView 的图片大小
